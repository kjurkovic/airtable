...
func Auth(l *log.Logger, dao *database.UserDao, config *config.Authentication) *AuthHandler {
	return &AuthHandler{
		logger: l,
		dao:    dao,
		config: config,
...
func (handler *AuthHandler) ForgetPassword(rw http.ResponseWriter, r *http.Request) {
	...
	// TODO: send notification event for email - rpc
...
func (handler *AuthHandler) generateAuthResponse(user *models.User, rw http.ResponseWriter) *models.Auth {
	expirationTime := time.Now().Add(handler.config.AccessTokenExpiration * time.Minute)
	...
	tokenString, err := token.SignedString([]byte(handler.config.SecretKey))
...
func (handler *AuthHandler) generateRefreshToken(user *models.User) string {
	...
	sb.WriteString(handler.config.SecretKey)
